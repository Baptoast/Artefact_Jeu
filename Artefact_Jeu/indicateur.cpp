...
void Indicateur::updateIndicateur(int numTy, int x, int y, vector<int>& listeMurCaseX, vector<int>& listeMurCaseY) {
	numType = numTy;
	posJoueurX = x;
	posJoueurY = y;
	if (numTy == 0) {
...
void Indicateur::afficheIndicateur(RenderWindow& window) {
	...
	for (int i = 0; i < listeCasePossible.size(); i++) {
		window.draw(listeCasePossible.at(i));
	}
	if (listeCasePossible.size() != 0) {
...
bool Indicateur::casePossible(RenderWindow& window) {
	...
	for (int i = 0; i < listeCasePossible.size(); i++) {
		if (listeCasePossible.at(i).getGlobalBounds().contains(positionSourisX, positionSourisY)) {
			return true;
		}
	}
	return false;
